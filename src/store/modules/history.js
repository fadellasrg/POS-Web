...
const moduleHistory = {
  ...
  actions: {
    actionGetHistoryFromAPI (context, data) {
      axios.get(`${context.rootState.setURL}/api/history?searchParams=&search=${data.search}&param=&sort=desc&page=${data.page}&limit=`, { headers: { token: context.rootState.auth.token } }).then((response) => {
    ...
    actionDetailHistory (context, data) {
      axios.get(`${context.rootState.setURL}/api/detailHistory/${data}`, { headers: { token: context.rootState.auth.token } }).then((response) => {
    ...
    insertHistory (context, data) {
      return new Promise((resolve, reject) => {
        axios.post(`${context.rootState.setURL}/api/history`, data, { headers: { token: context.rootState.auth.token } }).then((response) => {
