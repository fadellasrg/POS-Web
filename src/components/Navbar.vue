        ...
        <nav id="side-bar" class="navbar navbar-light">
            ...
            <b-modal ref="my-modalAdd" hide-footer id="modal-center" centered title="Add Item">
                <div style="height: 280px" class="container-fluid">
                    ...
                    <input type="text" v-model="form.image" class="form-control" placeholder="File Name">
                    <input @change="uploadFile($event)" type="file" class="form-control" placeholder="File">
                    <select v-model="form.id_category" style="margin-top: 10px; width: 100%" class="form-select form-select-lg mb-3">
...
export default {
  ...
  methods: {
    ...
    onAdd () {
      ...
      this.actionInsert(fd).then((response) => {
        alert(response)
        this.$router.go('/')
      }).catch((err) => {
        console.log(err)
